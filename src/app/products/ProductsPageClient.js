...
function ProductCard({ product }) {
  return (
    <div className="group rounded-3xl overflow-hidden border border-gray-100 hover:shadow-2xl transition-all duration-300 hover:-translate-y-1 bg-[#0b477b] flex flex-col">
      ...
      <div className="bg-white px-6 pt-4 pb-6" style={{ borderRadius: "0 0 40px 0" }}>
        <div className="w-10 h-10 rounded-xl bg-[#eef5fc] border border-[#d0e4f5] flex items-center justify-center mb-4">
          <svg className="w-5 h-5 text-[#0b477b]" fill="none" stroke="currentColor" strokeWidth="1.8" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" d={product.icon} />
          </svg>
        </div>
        <h3 className="text-[#0b477b] font-semibold text-2xl leading-tight">
          {product.name}
        </h3>
      </div>
        ...
        <div className="flex flex-col gap-4 flex-1">
          {product.features.map((f, i) => (
            <div key={i} className="flex items-start gap-3">
              ...
              <span className="text-white text-xs font-semibold leading-tight mt-1">{f.label}</span>
            </div>
          ))}
        </div>
